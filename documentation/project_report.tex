...
% ─── Dashed outer box ───
\node[dashedbox, minimum width=19cm, minimum height=8.5cm, fill=white] at (8.5, 0) {};
...
% Hidden state: h_{t-1} enters left side of top, h_t exits right side of top
\node[font=\footnotesize\itshape, anchor=south] (htprev) at (7.2, 2.8) {$h_{t-1}$};
\draw[arrow] (htprev.south) -- ([xshift=-0.3cm]gru.north);
\node[font=\footnotesize\itshape, anchor=south] (htcur) at (8.8, 2.8) {$h_t$};
\draw[arrow] ([xshift=0.3cm]gru.north) -- (htcur.south);
...
% ─── Output nodes: drive group (top), steer group (bottom) — no crossing ───
% Drive group
\node[neuron, fill=lightorange!50] (mu1)  at (13.5,  2.5) {};
\node[neuron, fill=lightred!50]    (sig1) at (13.5,  1.5) {};
% Steer group
\node[neuron, fill=lightorange!50] (mu2)  at (13.5, -0.5) {};
\node[neuron, fill=lightred!50]    (sig2) at (13.5, -1.5) {};

% Group labels
\node[font=\footnotesize, anchor=south] at (13.5,  3.1) {$[\mu_{\text{drive}},\;\log\sigma_{\text{drive}}]$};
\node[font=\footnotesize, anchor=north] at (13.5, -2.1) {$[\mu_{\text{steer}},\;\log\sigma_{\text{steer}}]$};
...
% ─── Action sampling nodes ───
\node[neuron, fill=accentorange!30, minimum size=0.5cm] (a1) at (16.0,  2.0) {};
\node[font=\footnotesize, right=0.15cm of a1] {$a_{\text{drive}}^t$};
\node[neuron, fill=accentorange!30, minimum size=0.5cm] (a2) at (16.0, -1.0) {};
\node[font=\footnotesize, right=0.15cm of a2] {$a_{\text{steer}}^t$};

% Sampling arrows — drive group to a_drive, steer group to a_steer (no crossing)
\draw[arrow] (mu1.east)  -- ++(0.4,0) |- (a1.west);
\draw[arrow] (sig1.east) -- ++(0.4,0) |- (a1.west);
\draw[arrow] (mu2.east)  -- ++(0.4,0) |- (a2.west);
\draw[arrow] (sig2.east) -- ++(0.4,0) |- (a2.west);
...
\centering
\begin{tikzpicture}[
    xscale=1.0, yscale=0.85, transform shape,
    mainbox/.style={rectangle, draw=black, thick, fill=white,
                    minimum height=1.1cm, align=center, font=\small},
    bluebox/.style={rectangle, draw=black, thick, fill=lightblue!35,
                    minimum height=1.1cm, align=center, font=\small},
    purplebox/.style={rectangle, draw=black, thick, fill=softpurple!30,
                      minimum height=1.1cm, align=center, font=\small},
    orangebox/.style={rectangle, draw=black, thick, fill=lightorange!40,
                      minimum height=1.1cm, align=center, font=\small},
    graybox/.style={rectangle, draw=black, thick, fill=lightgray!60,
                    minimum height=0.9cm, align=center, font=\small, rounded corners=2pt},
    greenbox/.style={rectangle, draw=black, thick, fill=softgreen!30,
                     minimum height=1.1cm, align=center, font=\small},
    innerbox/.style={rectangle, draw=gray!60, thick, fill=gray!10,
                     minimum height=0.7cm, align=center, font=\scriptsize},
    arrow/.style={-Stealth, thick},
    bluearrow/.style={-Stealth, thick, color=blue!60},
    every node/.style={font=\small}
]
...
% ──────────────────────────────────────────────────────────────
\node[font=\footnotesize, anchor=west] at (5.6, 6.8)
    {\tikz\draw[-Stealth,thick](0,0)--(0.55,0); \; DRL process};
\node[font=\footnotesize, anchor=west, text=blue!65] at (5.6, 6.1)
    {\tikz\draw[-Stealth,thick,blue!60](0,0)--(0.55,0); \; Curriculum update};

% ──────────────────────────────────────────────────────────────
% TOP-LEFT: gradient update label + Curriculum Scheduler box
% ──────────────────────────────────────────────────────────────
\node[font=\small\bfseries, anchor=west] at (-8.8, 7.2)
    {Updating parameters with gradient descent};
\node[font=\normalsize] at (-8.2, 6.3) {$\nabla \mathcal{L}(\theta)$};

\node[purplebox, minimum width=3.2cm, minimum height=1.4cm] (currsched)
    at (-6.8, 5.2) {Curriculum\\Scheduler};

% ──────────────────────────────────────────────────────────────
% TOP-RIGHT: Stochastic Environment
% ──────────────────────────────────────────────────────────────
\node[orangebox, minimum width=3.6cm, minimum height=2.2cm] (stochenv)
    at (3.5, 6.2) {};
\node[font=\small\bfseries, anchor=north] at (3.5, 7.2) {Stochastic};
\node[font=\small\bfseries] at (3.5, 6.6) {environment};
% Sim screenshot placeholder inside env box
\node[draw=gray!50, fill=gray!20, minimum width=3.0cm, minimum height=1.1cm,
      align=center, font=\scriptsize\itshape, text=gray!55, rounded corners=2pt]
      at (3.5, 5.75) {Newton + Isaac Lab\\simulation};

% ──────────────────────────────────────────────────────────────
% CENTER: Reward Function
% ──────────────────────────────────────────────────────────────
\node[mainbox, minimum width=2.8cm] (rewfunc) at (-2.0, 3.2) {Reward function};

% reward signal labels
\node[font=\footnotesize\itshape, text=blue!65] at (-4.5, 3.8) {$r_C$};   % curriculum reward
\node[font=\footnotesize\itshape] at (-2.0, 2.35) {$r_H$};                 % shaped reward

% ──────────────────────────────────────────────────────────────
% RIGHT: Clipped PPO (GRU) with internal diagram
% ──────────────────────────────────────────────────────────────
\node[mainbox, minimum width=5.2cm, minimum height=4.2cm] (ppo) at (3.5, 2.0) {};
\node[font=\small\bfseries, anchor=north] at (3.5, 4.0) {Clipped PPO (GRU)};

% Internal PPO diagram: actor + critic + GRU blocks
\node[innerbox, minimum width=1.4cm] (actor)  at (2.6, 2.6) {Actor\\(GRU)};
\node[innerbox, minimum width=1.4cm] (critic) at (4.4, 2.6) {Critic\\(GRU)};
\node[innerbox, minimum width=3.2cm] (ppoloss) at (3.5, 1.4)
    {PPO clip loss + value loss\\+ entropy bonus};

\draw[-Stealth, thin, gray!70] (actor.south) -- ++(0,-0.25) -| (ppoloss.north west);
\draw[-Stealth, thin, gray!70] (critic.south) -- ++(0,-0.25) -| (ppoloss.north east);

% Gradient update arrow from PPO back to curriculum (top loop)
\draw[-Stealth, thick, dashed, color=black!60]
    (ppo.north) -- ++(0, 0.6) -| (currsched.north)
    node[midway, above, font=\scriptsize\itshape] {progress signal};

% ──────────────────────────────────────────────────────────────
% LEFT: Expert sample (curriculum config) + Experience replay pool
% ──────────────────────────────────────────────────────────────
\node[bluebox, minimum width=2.0cm, minimum height=1.2cm] (expert)
    at (-8.2, 2.5) {Curriculum\\config};

\node[graybox, minimum width=1.2cm, minimum height=2.8cm, rotate=90] (replay)
    at (-5.8, 1.0) {Experience replay pool};

% Expert → replay
\draw[arrow] (expert.east) -- ++(0.6,0) |- (replay.east);

% Replay → reward function
\draw[arrow] (replay.west) -- ++(0,-0.0) -| (rewfunc.south);

% ──────────────────────────────────────────────────────────────
% ARROWS: environment → (s_t,a_t)_π  → reward
% ──────────────────────────────────────────────────────────────
% env to reward (left branch): (s_t,a_t)_π — exits stochenv.south, goes left then down
\draw[arrow] (stochenv.south) --
    node[midway, above, font=\footnotesize\itshape] {$(s_t, a_t)_\pi$}
    (-2.0, 5.1) -- (rewfunc.north);

% env to PPO (right branch): (s_t,a_t) — direct arrow with midpoint label
\draw[arrow] (stochenv.south) -- (ppo.north)
    node[midway, right=3pt, font=\footnotesize\itshape] {$(s_t,\,a_t)$};

% reward → PPO: r_t
\node[font=\footnotesize\itshape] at (0.5, 2.8) {$r_t$};
\draw[arrow] (rewfunc.east) -- ++(1.2, 0) |- (ppo.west);

% Curriculum → env (blue dashed) — exits currsched.north, goes up then right to stochenv.west
\draw[bluearrow, dashed] (currsched.north) |- (stochenv.west);

% Curriculum → reward weights (blue dashed)
\draw[bluearrow, dashed] (currsched.south) -- ++(0,-0.8) -| (rewfunc.north west);

% Curriculum config → curriculum scheduler
\draw[arrow] (expert.north) -- ++(0, 1.5) -| (currsched.south);

% ──────────────────────────────────────────────────────────────
% BOTTOM: experience tuple + experience sequence
% ──────────────────────────────────────────────────────────────
\node[font=\footnotesize\itshape] (exptuple) at (3.5, -0.6)
    {$(s_t,\,a_t,\,r_t,\,s_{t+1})$};
\draw[arrow] (ppo.south) -- (exptuple.north);

\node[graybox, minimum width=11.5cm, minimum height=0.85cm] (expseq)
    at (-1.0, -2.2) {Experience sequence (maximum rollout length $T$, GRU subsequence $= 16$)};

\draw[arrow] (exptuple.south) -- ++(0,-0.45) -| (expseq.north -| exptuple);

% Replay pool feeds PPO
\draw[arrow] (expseq.north west) -- ++(0,0.6) -| (replay.north);

\node[font=\scriptsize\itshape, text=black!55, align=center] at (-1.0, -3.35)
    {$\bigl[(s_{t-T},a_{t-T},r_{t-T},s_{t-T+1}),\;
       (s_{t-T+1},a_{t-T+1},r_{t-T+1},s_{t-T+2}),\;
       \cdots,\;
       (s_{t-1},a_{t-1},r_{t-1},s_t),\;
       (s_t,a_t,r_t,s_{t+1})\bigr]$};

\end{tikzpicture}
\caption{Block diagram of the reward-shaped DRL training loop. The curriculum scheduler (replacing GAIL's discriminator) adjusts sinkage depth and reward weights based on training progress. Shaped reward $r_t = r_H$ (hand-crafted) combines progress, escape milestones, rocking, and penalty terms. The GRU-PPO agent is updated via clipped surrogate loss with entropy bonus.}
