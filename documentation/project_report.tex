...
\node[inner, minimum width=4.8cm, minimum height=1cm] (loss) at (4, 0.3) 
    {PPO clip loss + value loss + entropy bonus};
\node[font=\tiny, text=gray!70, anchor=north] at (loss.south) {Updating parameters with Adam/SGD};
...
\draw[dashedblue] (sched.south) -- ++(0, -0.8) -| (reward.north)
    node[pos=0.7, left, text=blue!60] {Update weights};

% B. Environment → Reward
\draw[arrow] (env.west) -- ++(-1.5,0) |- (reward.north)
    node[pos=0.6, right, font=\footnotesize] {$(s_t, a_t)_\pi$};

% C. RL Closed Loop: Environment ↔ PPO
\draw[arrow] (env.south) -- (ppo.north |- env.south)
    node[midway, left, font=\footnotesize] {$\mathbf{s}_t$};
\draw[arrow] (ppo.north east) -- ++(1.3,0) |- (env.east)
    node[pos=0.3, right, font=\footnotesize] {$\mathbf{a}_t$};

% D. Reward → PPO
\draw[arrow] (reward.east) -- (ppo.west |- reward.east)
    node[midway, above, font=\footnotesize] {$r_t$};

% E. PPO → Experience Sequence
\draw[arrow] (ppo.south) -- ++(0, -1.2) -| (seq.north)
    node[pos=0.6, right, font=\footnotesize] {$(s_t, a_t, r_t, s_{t+1})$};
