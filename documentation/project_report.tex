...
Meraj Hossain Promit\\[0.3em]
\&\\[0.3em]
Chandak Chakma\\[1.2cm]
...
We couple the Newton physics engine's Material Point Method (MPM)~\cite{sulsky1994particle} with Isaac Lab's~\cite{isaaclab} rigid-body solver to produce physically accurate granular soil behaviour under Martian gravity ($3.72\,\text{m/s}^2$). A recurrent policy trained with Proximal Policy Optimization (PPO)~\cite{schulman2017ppo} and Gated Recurrent Units (GRU)~\cite{cho2014gru} operates on a 29-dimensional proprioceptive state comprising wheel velocities, slip ratios, IMU readings, drive torques, and dual entrapment detection flags. Curriculum learning progressively increases wheel sinkage from 6\,cm to 12\,cm across 64 parallel training environments on a single workstation (Intel i7-1255U, NVIDIA RTX 5070 Ti).

Training over 200{,}000 timesteps achieves an 80\% escape rate with a mean forward velocity of 0.25\,m/s and full curriculum completion. The policy develops emergent escape behaviours including coordinated rocking manoeuvres and asymmetric wheel-steering sequences not explicitly programmed. Planned next steps cover scaling to 2M+ timesteps, a CNN-GRU sinkage detection classifier, and on-rover deployment via ONNX Runtime on a Raspberry Pi 5 at 10\,Hz.
\end{abstract}
...
\textbf{Training load} & $\sim$10.2\,GB RAM used during 64-environment training \\
\textbf{Parallel envs} & 64 (RTX 5070 Ti, 16\,GB VRAM sufficient) \\
