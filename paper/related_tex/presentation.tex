...
\usepackage{xcolor}
\usepackage{graphicx}
\graphicspath{{../figures/}{../../experiments/regolith_recovery/plots/}}
\usepackage{array}
\usepackage{ragged2e}
\usepackage{etoolbox}
...
\begin{tcolorbox}[mycard, title={\faRadiation\; Obj 2: Entrapment Detection}]
  \scriptsize
  Dual-mode scheme: \keyword{slip-ratio counter} +
  \keyword{slip-anomaly gate}: no GPS, no external sensing
\end{tcolorbox}
\vspace{0.25em}
...
\begin{tcolorbox}[mycard, title={\faGraduationCap\; Obj 4: Curriculum Learning}]
  \scriptsize
  Progressive sinkage: \keyword{15 cm → 28 cm} across 16 parallel
  environments; competence-gated to prevent premature advancement
\end{tcolorbox}
\vspace{0.25em}
...
\begin{tabularx}{\textwidth}{@{}l l X c c c@{}}
  ...
  \textbf{Ours} & \textbf{6-wheel RB} & \textbf{in-engine baseline-validated} &
    \teal{\textbf{MPM}} & \teal{\textbf{Mars}} & \textbf{91--95\%} \\
  \bottomrule
\end{tabularx}
}
...
\begin{tabularx}{\linewidth}{@{}l X X@{}}
  ...
  \rowcolor{CardBg}
  Steps & 4M & 300K$\times$16 \\
  \rowcolor{SlideGray}
  Reward & GAIL+expert & \teal{Handcrafted} \\
  \rowcolor{CardBg}
  Escape & \textbf{90\%} & \teal{\textbf{91--95\%}} \\
  \rowcolor{SlideGray}
...
\begin{tcolorbox}[highlight]
  \tiny
  \textbf{\faExclamationTriangle\; Their edge:} real-world validated, GAIL+expert demos (we match on $\sim$4.8M aggregate env steps)
\end{tcolorbox}
\vspace{0.2em}
\begin{tcolorbox}[greenbox]
  \tiny
...
\node[innerbox, minimum width=7cm] (terrain) at (0, 11.0) {Randomized Sinkage Depth (Competence-Gated Curriculum)};
...
\node[innerbox, minimum width=4cm] (handrew) at (-3, 7.5) {Shaped Reward\\Module ($R_v$)};
...
\node[innerbox, minimum width=8cm, minimum height=0.7cm] (drl) at (0, 4.7) {Deep Reinforcement Learning (PPO-GRU, 16 parallel envs)};
...
\begin{tcolorbox}[mycard, title={\faCubes\; Rover on MPM Regolith Bed (Newton Sim)}]
  \centering\vspace{1pt}
  \includegraphics[height=2.5cm, width=0.92\linewidth, keepaspectratio]{paper_rover_entrapment}
\end{tcolorbox}
...
\begin{tcolorbox}[mycard, title={\faAtom\; Material Point Method (MPM)}]
  ...
  \begin{itemize}[leftmargin=1em, itemsep=2pt]
    ...
    \item Models sinkage progression, bulldozing, particle
      redistribution during wheel spin (rigid approx.\ cannot)
    \item \teal{$\sim$497,000 particles per environment} (16 envs $\Rightarrow$ $\sim$7.95M total)
  \end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[mycard, title={\faCubes\; Sand Bed Configuration}]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Voxel / PPC  & 0.05 m / 2.0 ($141{\times}141{\times}25$/env) \\
    Friction $\mu$ & $\mathcal{U}(0.6, 0.9)$ per-env per reset \\
    Gravity      & $(0,\, 0,\, -3.72)$ m/s² \\
    $k_e$ (sand) & $10^{8}$ Pa (physical dry sand) \\
...
\node[layer, fill=lightred!30, draw=marsred] (mpm) at (0, 1.2)
    {\textbf{Granular Solver}: SolverImplicitMPM ($\sim$497K particles, $\mu{\sim}\mathcal{U}(0.6,0.9)$)};
...
\begin{tcolorbox}[statbox]
  ...
  {\color{ArcticTeal}\bfseries\large 16} parallel envs $\cdot$
  {\color{MarsLight}\bfseries\large $\sim$497K} MPM particles each
\end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{columns}[T]
  ...
  \begin{tcolorbox}[enhanced,
    ...
    \textcolor{StarWhite}{\scriptsize
      $\mathbf{R_t = r_{prog} + r_{esc} + r_{esc,shaped} + r_{rock}}$
      $\mathbf{- p_{slip} - p_{tilt} - p_{smooth} - p_{abn} - p_{rev} - p_{hop} - p_{grind}}$
    }
  \end{tcolorbox}

  \vspace{0.25em}

  \begin{tcolorbox}[mycard, title={\faBalanceScale\; Reward Function $R_t$ (9-term, current)}]
  \vspace{2pt}
  {\tiny
  \begin{tabularx}{\linewidth}{@{}l r X@{}}
    ...
    $r_\text{prog}$ & $+1.5$ & World-frame fwd vel $\cdot\,\hat{\mathbf{e}}$ (always on, never gated) \\
    \rowcolor{AccentGreen!8}
    $r_\text{esc}$   & $+20$  & Sparse milestones at 0.5/1.0/2.0/3.0 m \\
    \rowcolor{AccentGreen!8}
    $r_\text{esc,shaped}$ & $+5$ & Dense $\Delta$-distance signal per step \\
    \rowcolor{AccentGreen!8}
    $r_\text{rock}$  & $+0.5$ & World-frame $|\Delta v_\text{proj}| \cdot f_\text{ent}$ (bootstrap nudge) \\
    \midrule
    \rowcolor{MarsRust!5}
    ...
    \rowcolor{MarsRust!5}
    $p_\text{smooth}$   & $-0.01$ & Action jerk ($\|\Delta a\|_2$); cut from $0.12$ for rocking \\
    \rowcolor{MarsRust!5}
    $p_\text{abn}$      & $-0.3$ & Reversing under slip anomaly $f_\text{anom}$ \\
    \rowcolor{MarsRust!5}
    $p_\text{rev}$      & $-1.0$ & Backward drift ($-\mathbf{v}_{xy}\!\cdot\!\hat{\mathbf{e}}$) \\
    \rowcolor{MarsRust!5}
    $p_\text{hop}$      & $-0.5$ & Vertical bounce $|v_z|$ (gated by burial grace) \\
    \rowcolor{MarsRust!5}
    $p_\text{grind}$    & $-0.05$ & Slip-conditional grinding ($\bar\sigma{>}0.70 \wedge \|a_d\|$) \\
    \bottomrule
  \end{tabularx}
  }
  \vspace{3pt}
  \begin{tcolorbox}[highlight]
    \tiny\centering
    \textbf{Key design:} all velocity terms world-frame projected on $\hat{\mathbf{e}}$ (no body-frame farming);
    $p_\text{grind}$ cut $0.30{\to}0.05$ (avoid zero-drive collapse), $p_\text{smooth}$ cut $0.12{\to}0.01$;
    $p_\text{hop}$ kills bounce-exploit; $p_\text{rev}$ breaks backward-drift local optimum
  \end{tcolorbox}
  \end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[mycard, title={\faTachometerAlt\; Formal Entrapment Criterion}]
  \scriptsize
  Wheel entrapment is the \textbf{conjunction}:
  \[
    f_\text{ent} = 1 \iff
    \begin{cases}
    ...
    \end{cases}
  \]
  where $\bar{\sigma}_\text{slip} = \frac{1}{6}\sum_{i}\!\left(1 - \frac{v_{\text{lin},i}}{\omega_i r}\right)$;
  $f_\text{ent}$ is pre-set to 1 at reset (rover spawns buried 15--28 cm).
\end{tcolorbox}

\vspace{0.25em}
\begin{tcolorbox}[mycard, title={\faSlidersH\; Dual Detection Mechanisms}]
  \scriptsize
  \teal{\faCircle} \textbf{Slip-based:}\;
  \resizebox{0.72\linewidth}{!}{$f_\text{ent} = \mathbb{1}\!\left[|v_x|<0.15 \wedge \bar\sigma>0.4\;\text{(15 steps)}\right]$}
  \vspace{3pt}

  \rust{\faCircle} \textbf{Slip-anomaly (leaky AND-gate):}\\[2pt]
  \resizebox{\linewidth}{!}{$f_\text{anom} = \mathbb{1}\!\left[\bar\sigma>0.92 \wedge |v_x|<0.20\;\text{for 15 steps}\right]$}
\end{tcolorbox}
...
\node[dbox] (det) at (0, 2.9)
    {\textbf{Dual-Mode Detector}\\
     {\scriptsize slip-ratio counter $+$ slip-anomaly gate}\\
     {\tiny $f_\text{ent},\; f_\text{anom}$ \ (Alg.\,1)}};
...
\begin{tcolorbox}[greenbox]
  \tiny
  \textbf{Competence gate} (50-ep, fail-closed): advance only when escape
  $\geq0.50$ \emph{and} trivial $<0.25$; back off if escape $<0.20$
\end{tcolorbox}
...
\begin{tcolorbox}[mycard, title={\faServer\; Training Hardware}]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    CPU & Intel i7 (MuJoCo rigid bodies on CPU) \\
    Envs & 16 parallel ($\sim$497K particles each) \\
    Throughput & $\sim$340 policy steps/s ($\sim$65\% MPM solve) \\
    Wall time & $\sim$39 hrs for 300K timesteps (0.60 m bed) \\
  \end{tabular}
\end{tcolorbox}
...
\begin{tcolorbox}[highlight]
  \scriptsize
  \textbf{Domain randomization:} sand $\mu\,{\sim}\,\mathcal{U}(0.6,0.9)$,
  motor gain $\mathcal{U}(0.92,1.08)$, encoder/IMU/GPS sensor noise, spawn yaw $\pm5°$
\end{tcolorbox}
\end{columns}
\end{frame}
...
\node[font=\small, text=black!70, align=center] at (4, 5.5) {Newton MPM +\\MuJoCo (16 envs)};
...
\begin{frame}{Training Overview: 300K Timesteps (seed 1)}
\vspace{-0.5em}
\begin{columns}[T]
\column{0.50\textwidth}
\begin{tcolorbox}[mycard, title={\faChartLine\; Training Dashboard (PPO-GRU, 300K steps)}]
  \vspace{2pt}
...
\begin{tcolorbox}[statbox]
  ...
  {\color{DimText}\tiny Training-time escape rate (curriculum-avg)}\\
  {\color{SandGold}\tiny $0.869 \pm 0.002$ final 20K steps — \emph{not} the validation rate}
\end{tcolorbox}
\vspace{0.1em}
\begin{tcolorbox}[statbox]
  \centering
...
\begin{tcolorbox}[mycard]
  \scriptsize\centering
  \textbf{\color{ArcticTeal}Curriculum at 75\%} at 300K steps;\\
  gate-limited — full sinkage (28 cm) not yet reached
\end{tcolorbox}
\column{0.33\textwidth}
\begin{tcolorbox}[highlight]
  \scriptsize\centering
  \textbf{2.0 m still climbing} at 300K;\\
  3.0 m escape bounded by bed size\\
  (rear wheels clear sand at ${\sim}0.9$ m)
\end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[mycard, title={\faChartArea\; Reward Convergence (Isotonic Envelope)}]
  \vspace{1pt}
  \includegraphics[width=\linewidth, height=2.3cm, keepaspectratio, trim=0 8 0 8, clip]{reward_convergence}
\end{tcolorbox}
...
\column{0.38\textwidth}
\begin{tcolorbox}[mycard, title={\faRobot\; Learned Strategy: \emph{Steered Ploughing}}]
  \scriptsize
  \begin{itemize}[leftmargin=1em, itemsep=1pt]
    \item[\rust{\faCircle[regular]}] Drive saturated \keyword{forward} ($69$--$85\%$); $\leq\!13\%$ reverse
    \item[\rust{\faCircle[regular]}] Rocking signature (drive sign flip) $\leq\!2\%$ of steps
    \item[\rust{\faCircle[regular]}] Corner steer at \keyword{full lock $65$--$71\%$} --- needs 10D
  \end{itemize}
\end{tcolorbox}

\vspace{0.2em}
\begin{tcolorbox}[mycard, title={\faChartBar\; Steering vs.\ Rocking (2 best seeds)}]
  \vspace{2pt}
  \includegraphics[width=\linewidth, height=2.0cm, keepaspectratio, trim=0 8 0 8, clip]{policy_action_behavior}
\end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[greenbox]
  \tiny
  \textbf{\tick\ Validated in-engine:} best seeds escape \textbf{95.0\% / 90.8\%}
  vs 30.8\% constant drive, 0\% rocking (120 trials each). Next: 4M-step RTX\,4090 run.
\end{tcolorbox}
...
\begin{tcolorbox}[mycard, title={\faStar\; What Worked Well}]
  \tiny
  \begin{itemize}[leftmargin=0.8em, itemsep=2pt]
    \item[\tick] 86.9\% training-time; return $-20.7\!\to\!{+}35.1$, peak $+67.0$
    \item[\tick] In-engine validation: 90.8--95.0\% vs 30.8\% / 0\% baselines
    \item[\tick] Steered-ploughing strategy ($65$--$71\%$ full-lock steer)
    \item[\tick] Entropy $>$0.5\,nats — no premature convergence
    \item[\tick] sim2sim pilot: 60\% recovery, H1 confirmed ($0.04°$ heading)
  \end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[mycard, title={\faExclamationTriangle\; Known Limitations}]
  ...
  \begin{itemize}[leftmargin=0.8em, itemsep=2pt]
    ...
    \item[\warn] In-engine only — no cross-engine / hardware transfer yet
  \end{itemize}
\end{tcolorbox}

\vspace{0.2em}
\begin{tcolorbox}[mycard, title={\faTable\; Failure-Mode Taxonomy (13.1\% non-escape)}]
  {\tiny\setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lcc@{}}
    \toprule
    \rowcolor{MarsRust!8}
    \color{StarWhite}\textbf{Mode} & \color{StarWhite}\textbf{of episodes} & \color{StarWhite}\textbf{of failures} \\
    \midrule
    \rowcolor{CardBg} Stall (recovery, no yield) & $8.7\%$ & $66.5\%$ \\
    \rowcolor{SlideGray} Timeout (40\,s budget)     & $3.1\%$ & $23.6\%$ \\
    \rowcolor{CardBg} Lateral OOB (env guard)    & $1.3\%$ & $9.9\%$ \\
    \bottomrule
  \end{tabular}
  }
\end{tcolorbox}
...
%% ── SLIDE 14b: VALIDATION RESULTS (headline) ─────────────────
\begin{frame}{Validation: In-Engine Baseline Comparison}
\vspace{-0.2em}
\begin{columns}[T]
\column{0.40\textwidth}
\begin{tcolorbox}[mycard, title={\faFlask\; Controlled Protocol}]
  \scriptsize
  \begin{itemize}[leftmargin=1em, itemsep=2pt]
    \item Same engine the policy trained in (Newton MPM, AAU rover, Mars $g$)
    \item \textbf{120 trials} per controller (30 each at sinkage 0.15/0.20/0.25/0.28\,m)
    \item Scored by the env's \keyword{pre-reset} escape flag (proj.\ progress $>3.0$\,m)
    \item Solver-blowup episodes conservatively counted as failures
  \end{itemize}
\end{tcolorbox}
\vspace{0.2em}
...
\begin{tabularx}{\linewidth}{@{}X c c@{}}
  \toprule
  \rowcolor{MarsRust!8}
  \color{StarWhite}\textbf{Controller} &
  \color{StarWhite}\textbf{Escape} &
  \color{StarWhite}\textbf{95\% CI} \\
  \midrule
  \rowcolor{MarsRust!5}
  Scripted rocking     & $0.0\%$  & $[0,0]$ \\
  \rowcolor{SlideGray}
  Constant drive       & $30.8\%$ & $[23,39]$ \\
  \rowcolor{AccentGreen!10}
  \textbf{Policy (seed 1)} & $\mathbf{95.0\%}$ & $[91,98]$ \\
  \rowcolor{AccentGreen!10}
  \textbf{Policy (seed 3)} & $\mathbf{90.8\%}$ & $[86,96]$ \\
  \bottomrule
\end{tabularx}
}
\vspace{0.3em}
\begin{tcolorbox}[greenbox]
  \scriptsize\centering
  \tick\ $3\times$ margin over constant drive, \emph{non-overlapping} CIs ---
  isolates the policy from the soil draw
\end{tcolorbox}

\column{0.58\textwidth}
\begin{tcolorbox}[mycard, title={\faChartBar\; Escape Rate by Controller}]
  \vspace{1pt}
  \includegraphics[width=\linewidth, height=2.45cm, keepaspectratio, trim=0 6 0 6, clip]{escape_comparison}
\end{tcolorbox}
\vspace{0.1em}
\begin{tcolorbox}[mycard, title={\faLayerGroup\; Escape Rate vs.\ Initial Sinkage Depth}]
  \vspace{1pt}
  \includegraphics[width=\linewidth, height=2.45cm, keepaspectratio, trim=0 6 0 6, clip]{escape_vs_sinkage}
\end{tcolorbox}
\end{columns}
\end{frame}

%% ── SLIDE 14c: OMNIDIRECTIONAL + SEED VARIANCE ───────────────
\begin{frame}{Omnidirectional Recovery \& Training Stability}
\vspace{-0.2em}
\begin{columns}[T]
\column{0.52\textwidth}
\begin{tcolorbox}[mycard, title={\faCompass\; Top-Down Terminal Positions (true)}]
  \vspace{1pt}
  \includegraphics[width=\linewidth, height=4.3cm, keepaspectratio, trim=0 6 0 6, clip]{topdown_escape_heatmap}
\end{tcolorbox}
\vspace{0.1em}
\begin{tcolorbox}[highlight]
  \tiny\centering
  \textbf{Left:} constant drive — sparse escapes, dense failure cluster inside the bed.\\
  \textbf{Right:} policy forms a uniform escaped ring across all bearings ($\phi_\text{esc}\!\sim\!\mathcal{U}(0,2\pi)$).
\end{tcolorbox}

\column{0.48\textwidth}
\begin{tcolorbox}[mycard, title={\faRandom\; Bimodal Seed Outcomes}]
  \scriptsize
  \begin{itemize}[leftmargin=1em, itemsep=2pt]
    \item[\tick] \keyword{3 of 5 seeds} reach competent recovery ($\geq\!84\%$)
    \item[\rust{\faCircle[regular]}] Seeds 1, 3 climb to deepest curriculum (0.83, 0.94)
    \item[\warn] Seed 0 \emph{collapses} (69\% peak $\to$ 10\%)
    \item[\warn] Seed 4 \emph{never bootstraps} past the competence gate
  \end{itemize}
  \vspace{2pt}
  Outcomes are bimodal (converge vs.\ fail), so we report \keyword{per-seed} numbers, not a misleading mean.
\end{tcolorbox}
\vspace{0.15em}
\begin{tcolorbox}[mycard, title={\faChartLine\; Escape-Rate Curves (5 seeds)}]
  \vspace{1pt}
  \includegraphics[width=\linewidth, height=2.4cm, keepaspectratio, trim=0 6 0 6, clip]{seed_variance}
\end{tcolorbox}
\end{columns}
\end{frame}
...
\begin{tcolorbox}[mycard, title={\faSitemap\; Three-Layer Dual-Brain Handoff}]
  \scriptsize
  \begin{enumerate}[leftmargin=1.4em, itemsep=2pt, label={\color{MarsLight}\bfseries\arabic*.}]
    \item \keyword{PD Nav Brain} drives A$\to$B (no sand awareness)
    \item \keyword{Detector} ($f_\text{ent}$ 15 steps) hands over to escape
    \item \keyword{GRU-PPO} escapes toward GPS bearing (zero-shot), then returns control
  \end{enumerate}
\end{tcolorbox}

\vspace{0.15em}
\begin{tcolorbox}[mycard, title={\faExclamationTriangle\; Two Goal-Completion Failure Modes}]
  \tiny
  \begin{itemize}[leftmargin=1em, itemsep=2pt]
    \item[\warn] \textbf{Stall in bed:} high-sinkage ($d{\geq}0.25$\,m) tail not consolidated at 300K steps
    \item[\warn] \textbf{Post-escape re-entrapment:} PD pure-pursuit steers back across bed edge
  \end{itemize}
  \vspace{2pt}
  \textit{Fix (no retraining):} post-escape waypoint offset $0.5$\,m from bed edge.
\end{tcolorbox}

\vspace{0.15em}
\begin{tcolorbox}[highlight]
  \tiny\centering
  \textbf{\warn H2 partial / H3 pending:} full $N{=}100\times5$-seed study planned for camera-ready;
  recovery-rate gap (60\% vs 86.9\%) targets the 4M-step run.
\end{tcolorbox}
...
\begin{tikzpicture}[scale=0.9, font=\scriptsize,
  ...
  \foreach \y/\item in {
      3.9/{Scale to 4M steps (RTX 4090)},
...
\begin{tcolorbox}[mycard, title={\faStar\; Technical Contributions}]
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=5pt,
    ...
    \item \keyword{Novel} dual-mode detection: slip-ratio counter +
      slip-anomaly gate, no external sensing
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  %% Only "Thank You" centred on the slide
  \node[text=StarWhite, font=\fontsize{56}{64}\selectfont\bfseries]
