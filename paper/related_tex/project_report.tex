...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small RL-Based Wheel Entrapment Recovery for Mars Rovers}
...
% DU Logo
\includegraphics[width=3cm]{/home/mhpromit7473/regolith_entrapment_research/documentation/figures/du_logo.png}\\[0.8em]

{\Large\textbf{University of Dhaka}}\\[0.4em]
{\large\textbf{Department of Robotics and Mechatronics Engineering}}\\[1.2cm]

Course Code: \textbf{RME 4100 (Project)}\\[1.0cm]

{\large\textbf{Adaptive Neural Control for Mars Rover Wheel\\[0.3em]
Entrapment Recovery in Granular Terrain}}\\[1.2cm]

Meraj Hossain Promit \\[0.3cm] [Exam Roll- 139123]\\[0.3em]
\&\\[0.3em]
Chandak Chakma \\[0.3cm] [Exam Roll- 139124]\\[1.2cm]

{\large\textbf{Under the supervision of}}\\[2.2cm]
\vspace{1.8cm}
\begin{minipage}[t]{0.45\textwidth}
\centering
\rule{0.75\linewidth}{0.8pt}\\[0.5em]
\textbf{Supervisor}\\
...
Exam Roll- 139123 \\[0.3cm] [Reg. No-2021711637]\\[0.3em]
\&\\[0.3em]
Exam Roll- 139124 \\[0.3cm] [Reg. No-2021211416]\\[1.2cm]
...
\noindent
Above all, we are profoundly grateful to Almighty Allah for bestowing upon us the strength, patience, and clarity of thought needed to carry this research through to completion.

\vspace{0.8em}
\noindent
We owe our deepest thanks to our parents, whose unconditional love, enduring sacrifice, and steadfast belief in us have been the foundation of everything we have achieved. Their encouragement has been our constant motivation.

\vspace{0.8em}
\noindent
We are sincerely grateful to our supervisor, \textbf{Md.\ Jubair Ahmed Sourov}, and our co-supervisor, \textbf{Dr.\ Sejuti Rahman}, for their expert guidance, constructive feedback, and unwavering support at every stage of this work. Their deep knowledge of robotics, mechatronics, and intelligent systems significantly shaped our understanding of physics-aware reinforcement learning and autonomous navigation.

\vspace{0.8em}
\noindent
We also wish to express our appreciation to \textbf{Dr.\ Abhishek Kumar Ghosh} for his Research Methodology and Technical \& Scientific Writing Lab, which laid the groundwork for our approach to structured research and academic communication, and whose enthusiasm for the field of reinforcement learning inspired us to explore it deeply.

\vspace{0.8em}
\noindent
We gratefully acknowledge the broader open-source and educational communities whose resources proved invaluable throughout this project. In particular, the \textbf{Hugging Face Deep Reinforcement Learning Course} provided an excellent foundation in modern RL algorithms and best practices. \textbf{NVIDIA's Isaac Lab and Newton documentation}, along with NVIDIA's developer resources on GPU-accelerated simulation and Warp, were indispensable references for the physics simulation stack. The \textbf{Grokking Deep Reinforcement Learning} book by Miguel Morales offered critical intuition behind policy gradient methods and curriculum design. Numerous \textbf{YouTube tutorials and open-source walkthroughs} on PPO, GRU architectures, and sim-to-real transfer helped bridge theory and implementation throughout the project.

\vspace{0.8em}
\noindent
We extend our thanks to all faculty members of the \textbf{Department of Robotics and Mechatronics Engineering}, University of Dhaka, for cultivating an environment of intellectual curiosity and rigorous academic inquiry.

\vspace{0.8em}
\noindent
Finally, this work stands as a testament to shared effort and mutual perseverance. The collaboration between us through every debugging session, design decision, and late-night experiment has been as formative as the research itself. We are grateful for every challenge that made us better engineers and researchers.

\vspace{1.5em}
\noindent
We sincerely thank everyone who contributed, directly or indirectly, to the completion of this work.
...
\begin{abstract}
\noindent
Wheel entrapment in granular regolith is one of the most critical failure modes for planetary rovers, as demonstrated by the permanent immobilization of NASA's Spirit rover in 2009~\cite{arvidson2011spirit}. This project develops an autonomous entrapment recovery system for a 6-wheel rocker-bogie Mars rover using deep reinforcement learning trained entirely in simulation.

We couple the Newton physics engine's Material Point Method (MPM)~\cite{sulsky1994particle} with Isaac Lab's~\cite{isaaclab} rigid-body solver to produce physically accurate granular soil behaviour under Martian gravity ($3.72\,\text{m/s}^2$). A recurrent policy trained with Proximal Policy Optimization (PPO)~\cite{schulman2017ppo} and Gated Recurrent Units (GRU)~\cite{cho2014gru} operates on a 29-dimensional proprioceptive state comprising wheel velocities, slip ratios, IMU readings, drive torques, and dual entrapment detection flags. Curriculum learning progressively increases wheel sinkage from 12\,cm to 22\,cm across 32 parallel training environments on a single workstation (Intel i7-1255U, NVIDIA RTX 5070 Ti).

Training over 200{,}000 timesteps achieves a 60\% escape rate with a mean forward velocity of 0.25\,m/s and full curriculum completion. The policy develops emergent escape behaviours including coordinated rocking manoeuvres and asymmetric wheel-steering sequences not explicitly programmed. The primary next step is \textbf{sim-to-sim validation}: transfer to a second simulator (MuJoCo standalone or Gazebo with terramechanics) across sloped terrain, variable soil density, and lunar gravity to confirm that escape behaviours are not engine-specific artefacts. Physical deployment on a Raspberry Pi 5 via ONNX Runtime at 10\,Hz is a secondary objective, contingent on successful sim-to-sim transfer and availability of the AAU rover hardware.
\end{abstract}
...
\begin{equation}
    R_t \;=\; \underbrace{r_{\text{progress}} + r_{\text{escape}} + r_{\text{rocking}}}_{\text{incentives}}
          \;-\; (\underbrace{p_{\text{slip}} + p_{\text{tilt}} + p_{\text{smooth}} + p_{\text{abnormal}})}_{\text{penalties} \;(\text{all positive, subtracted})}
...
\vspace{6pt}
\begin{tcolorbox}[objbox, colframe=headerblue, title={1.\enspace Realistic Simulation}]
...
\vspace{4pt}
\begin{tcolorbox}[objbox, colframe=headerblue, title={2.\enspace Entrapment Detection}]
...
\vspace{4pt}
\begin{tcolorbox}[objbox, colframe=headerblue, title={3.\enspace Recovery Policy}]
...
\vspace{4pt}
\begin{tcolorbox}[objbox, colframe=headerblue, title={4.\enspace Curriculum Learning}]
...
\vspace{4pt}
\begin{tcolorbox}[objbox, colframe=headerblue, title={5.\enspace Sim-to-Sim Validation \& Sim-to-Real Transfer \textit{(if possible)}}]
: \textbf{Primary ---} Validate that escape behaviours generalise beyond the Newton MPM engine
via cross-simulator transfer (MuJoCo standalone or Gazebo with terramechanics), multi-terrain
stress-testing (sloped beds $\pm15^\circ$, variable soil density $\rho\in[1200,1600]\,\text{kg/m}^3$,
lunar gravity), and multi-seed statistical evaluation.
\textbf{Secondary (contingent) ---} If sim-to-sim succeeds and AAU rover hardware is available,
export the GRU policy to ONNX and validate $<$5\,ms inference at 10\,Hz on a Raspberry Pi 5
...
% ─────── 2.0 Foundational Background ─────────────────────────────
\subsection{Foundational Concepts and Terminology}
...
\subsubsection*{Rocker-Bogie Suspension}
The rocker-bogie suspension is a passive linkage mechanism used on all NASA Mars rovers (Sojourner, Spirit, Opportunity, Curiosity, Perseverance) and our AAU platform. It consists of a \textit{rocker} arm connecting the front wheel to a \textit{bogie} sub-assembly bearing the middle and rear wheels. A differential link couples the two rocker arms so that the body roll is kept to half the terrain-induced tilt of any single rocker. This passive compliance allows the six wheels to maintain ground contact over obstacles up to twice the wheel diameter. The kinematic complexity, arising from passive joints, a differential, and no central actuator for body attitude, makes control significantly harder than a simple 4-wheel skid-steered platform.

\subsubsection*{Wheel--Soil Interaction and Slip Ratio}
When a driven wheel rotates in granular soil, the soil deforms and the wheel may sink (\textit{sinkage}) while the contact patch shears the granular medium. The \textit{slip ratio} $s \in [0, 1]$ quantifies this mismatch:
\begin{equation}
    s = 1 - \frac{v_{\text{linear}}}{\omega r}
\end{equation}
where $v_{\text{linear}}$ is the measured forward velocity, $\omega$ the wheel angular velocity, and $r$ the wheel radius. A slip ratio approaching 1 indicates the wheel is spinning freely with no forward progress, which is a hallmark of entrapment. Sustained high slip combined with low forward velocity is the primary detection signal used in this work.

\subsubsection*{Material Point Method (MPM)}
MPM~\cite{sulsky1994particle} is a hybrid Eulerian--Lagrangian continuum method that represents the material as a set of \textit{Lagrangian particles} (tracking mass, momentum, and deformation gradient) and a background \textit{Eulerian grid} (for solving the equations of motion). At each timestep, particle data is transferred to the grid (\textit{P2G}), grid equations are solved, and updated velocities are mapped back to particles (\textit{G2P}). For granular sand, the Drucker--Prager yield surface~\cite{klar2016sand} governs plastic flow: material flows when the stress state reaches the yield cone, and elastically recovers otherwise. Compared to rigid-body approximations, MPM correctly models sinkage progression, bulldozing resistance, and particle redistribution during wheel spinning, all of which are critical for entrapment simulation.

\subsubsection*{Proximal Policy Optimization (PPO)}
PPO~\cite{schulman2017ppo} is an on-policy actor-critic RL algorithm that improves training stability by constraining the policy update step size via a clipped surrogate objective:
\begin{equation}
    \mathcal{L}^{\text{CLIP}}(\theta) = \mathbb{E}_t \left[ \min\!\left( r_t(\theta)\hat{A}_t,\ \text{clip}(r_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t \right) \right]
\end{equation}
where $r_t(\theta) = \pi_\theta(a_t|s_t) / \pi_{\theta_{\text{old}}}(a_t|s_t)$ is the probability ratio and $\hat{A}_t$ is the generalized advantage estimate~\cite{schulman2015gae}. The clip parameter $\epsilon$ (typically 0.2) prevents destructively large updates. PPO has become the default algorithm for continuous-control locomotion tasks due to its simplicity, sample efficiency, and robustness to hyperparameter choice.

\subsubsection*{Gated Recurrent Unit (GRU)}
A GRU~\cite{cho2014gru} is a recurrent neural network cell that maintains a hidden state $\mathbf{h}_t$ summarising history:
\begin{equation}
    \mathbf{h}_t = (1 - \mathbf{z}_t) \odot \mathbf{h}_{t-1} + \mathbf{z}_t \odot \tilde{\mathbf{h}}_t
\end{equation}
where $\mathbf{z}_t$ is the update gate and $\tilde{\mathbf{h}}_t$ is the candidate hidden state. GRUs are preferred over LSTMs~\cite{hochreiter1997lstm} when model size is constrained (e.g., deployment on a Raspberry Pi 5), as they achieve comparable sequence-modelling performance with roughly one-third fewer parameters. In our setting, the GRU enables the policy to track slow-developing entrapment dynamics (sinkage over several seconds) that would be invisible to a memoryless MLP policy.
...
\begin{enumerate}
    ...
    \item \textbf{No prior work combines a 6-wheel rocker-bogie rover with MPM granular physics and DRL.}
    Bi and Ding~\cite{bi2026escape} achieve strong escape performance but on a 4-wheel SSMR in rigid terrain. Jain et al.~\cite{jain2019} and Inotsume et al.~\cite{inotsume2020} target planetary rovers but use approximate terrain models. No existing work trains an escape policy for a rocker-bogie rover in a physics-accurate granular simulation.

    \item \textbf{Granular sinkage as an entrapment mode is unstudied in the DRL context.}
    All DRL-based recovery works (Bi \& Ding, Jain, Fankhauser) address entrapment or falls caused by rigid terrain geometry. Progressive granular sinkage, where the rover settles into soil over several seconds, is a qualitatively different failure mode requiring memory-based detection and recovery strategies that existing reward functions do not capture.

    \item \textbf{Mars gravity ($3.72\,\text{m/s}^2$) and its effect on traction has not been modelled in any DRL recovery work.}
    Reduced gravity lowers normal forces at the wheel--soil interface, reducing traction and changing the sinkage dynamics significantly. All reviewed DRL works train and test under Earth gravity. Our simulation explicitly sets $g = 3.72\,\text{m/s}^2$.

    \item \textbf{Independent per-wheel drive and steer control is unexplored for entrapment recovery.}
    ...
    \item \textbf{Curriculum-driven sinkage depth has not been applied to entrapment policy training.}
    ...
    \item \textbf{No existing work provides a complete sim-to-real pipeline for a rocker-bogie rover.}
    RLRoverLab~\cite{rlroverlab} provides navigation policies but no entrapment recovery. Bi and Ding demonstrate real-world transfer but for a different robot class. Our planned ONNX export and Raspberry Pi 5 deployment would be the first complete sim-to-real entrapment recovery pipeline for a 6-wheel rocker-bogie platform.
...
\begin{figure}[H]
\centering
\begin{tcolorbox}[dashedbox, left=6pt, right=6pt, top=6pt, bottom=6pt]
\centering
\begin{tikzpicture}[
...
\node[font=\scriptsize\itshape, text=gray!70] at (0.0, -4.5) {
...
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/mhpromit7473/regolith_entrapment_research/documentation/figures/rl_rover_lab_usd.png}
    \caption{AAU 6-wheel Mars rover USD asset from RLRoverLab}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{/home/mhpromit7473/regolith_entrapment_research/documentation/figures/rover_newton_mpm.png}
    \caption{Rover on MPM granular regolith bed in Newton simulation}
\end{subfigure}
\caption{The AAU Mars rover platform used in simulation. (a) USD model render showing the 6-wheel rocker-bogie suspension from RLRoverLab. (b) Rover in Newton simulation environment with granular regolith particles under Martian gravity (3.72\,m/s$^2$).}
...
\begin{tikzpicture}[
    node distance=0.5cm,
    layer/.style={rectangle, draw=gray!60, fill=white, thick, rounded corners=3pt,
                  minimum height=1.1cm, minimum width=11cm, align=center, font=\small},
    arrow/.style={-Stealth, thick, color=black},
    biarrow/.style={Stealth-Stealth, thick, color=black},
    scale=0.85, transform shape
]
\node[layer=headerblue] (rl)     at (0, 8.0) {\textbf{RL Agent Layer}: PPO-GRU Policy (25 Hz)};
\node[layer=accentorange] (env)  at (0, 6.0) {\textbf{Environment Layer}: Isaac Lab DirectRLEnv (50 Hz physics)};
\node[layer=softgreen] (rigid)   at (0, 4.0) {\textbf{Rigid-Body Solver}: MuJoCo CPU (4 substeps, elliptic friction)};
\node[layer=marsred] (mpm)       at (0, 2.0) {\textbf{Granular Solver}: SolverImplicitMPM (38K particles, $\mu=0.7$)};
\node[layer=softpurple] (ground) at (0, 0.0) {\textbf{Ground Plane}: Newton callback (no Nucleus dependency)};

% Two-way coupling between rigid and MPM
\draw[biarrow, line width=1.5pt] (rigid.south) -- (mpm.north)
    node[midway, right=0.3cm, font=\footnotesize] {Two-way coupling};

% Action arrow: rl → env (left side, downward)
\draw[arrow] ($(rl.south)+(-1.0,0)$) -- ($(env.north)+(-1.0,0)$)
    node[midway, left, font=\footnotesize] {$\mathbf{a}_t$};

% Observation arrow: env → rl (right side, upward)
\draw[arrow] ($(env.north)+(1.0,0)$) -- ($(rl.south)+(1.0,0)$)
    node[midway, right, font=\footnotesize] {$\mathbf{o}_t,\, r_t$};

% env → rigid (pass-through)
\draw[arrow] (env.south) -- (rigid.north);

% rigid → ground (pass-through)
\draw[arrow] (rigid.south) -- ($(mpm.north)+(0,0)$);
...
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/side_view.png}
    \caption{Side view of rover on regolith}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/isometric.png}
...
All 200{,}000-timestep experiments were conducted on a single workstation. Table~\ref{tab:hardware} summarises the system configuration.

\begin{tcolorbox}[dashedbox, left=0pt, right=0pt, top=4pt, bottom=4pt]
\begin{table}[H]
\centering
\caption{Training system configuration (Lenovo ThinkBook 14 G4 IAP)}
\label{tab:hardware}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{>{\raggedright}p{3.8cm} X}
\toprule
\rowcolor{gray!20}
\textbf{Component} & \textbf{Specification} \\
\midrule
\textbf{Operating System} & Ubuntu 22.04.5 LTS (x86\_64), Kernel 6.8.0-107-generic \\
\textbf{CPU} & 12th Gen Intel Core i7-1255U, 10 cores / 12 threads @ up to 4.7\,GHz (2P + 8E) \\
\textbf{GPU (Training)} & NVIDIA RTX 5070 Ti, 16\,GB GDDR7 (primary, Isaac Sim + MPM kernels) \\
\textbf{GPU (Display)} & Intel Iris Xe (integrated, desktop rendering only) \\
\textbf{RAM} & 40\,GB DDR5 (39{,}825\,MiB usable) \\
\textbf{Training load} & $\sim$10.2\,GB RAM used during 32-environment training \\
\textbf{Approx. wall time} & $\sim$6--8 hours for 200{,}000 timesteps \\
\bottomrule
\end{tabularx}
\end{table}
\end{tcolorbox}
...
\draw[ga] (dmd.west) -| (nav.north)
    node[pos=0.18, above, font=\small\bfseries, text=green!45!black] {No};
\draw[ra] (dmd.east) -| (esc.north)
    node[pos=0.18, above, font=\small\bfseries, text=red!55!black] {Yes};
\draw[ga] (nav.south) |- (mot.west);
\draw[ra] (esc.south) |- (mot.east);
\draw[da] (esc.west) -- (0, -0.3) -- (dmd.south)
    node[pos=0.75, right=3pt, font=\scriptsize, text=black!45] {escaped};
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/episode_dashboard.png}
\caption{Episode running data dashboard for a single evaluation episode (random policy baseline). Five panels: (top-left) wheel drive velocities with left/right differentiation; (top-right) drive motor torques FL, ML, RL, FR, MR, RR; (mid-left) IMU pitch angle; (mid-right) entrapment and torque anomaly binary flags; (bottom) XY trajectory on MPM sand with time-coloured path, escape milestone rings, and heading arrows. Format matches Bi and Ding~\cite{bi2026escape}, Figs.~18/19.}
...
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/rover_entrapment.png}
    \caption{Rover entrapment in simulation}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/escape_manuever.png}
...
% ── Priority 1 ──────────────────────────────────────────────────
\subsubsection{Priority 1: Immediate Next Steps}

These tasks are directly unblocked by current results and can be pursued in parallel.

\begin{itemize}
    \item \textbf{Scale training to 2M+ timesteps} on an RTX 4090 (512 parallel environments) targeting $>$90\% escape rate and deeper curriculum completion.
    \item \textbf{Hyperparameter sweep}: rollout length, entropy coefficient, learning rate schedule, and GRU hidden size following~\cite{schulman2015gae}.
    \item \textbf{Ablation studies}: GRU vs.\ MLP policy, dual-sensor detection vs.\ single, curriculum on vs.\ off, to isolate each contribution.
    \item \textbf{CNN-GRU sinkage detection classifier} (Phase 1): train on 10K auto-labeled simulation episodes, target F1~$>$~0.90 on the ``entrapped'' class with $<$1\,s latency.
    \item \textbf{GAIL reward fusion}~\cite{ho2016gail} following Bi and Ding~\cite{bi2026escape}: blend handcrafted rewards with a discriminator signal ($R = 0.9R_H + 0.1R_D$) to smooth escape trajectories.
    \item \textbf{Asymmetric critic} with privileged terrain information (soil friction, sinkage depth) available only at training time, following RMA~\cite{kumar2021rma}.
\end{itemize}

% ── Priority 2 ──────────────────────────────────────────────────
\subsubsection{Priority 2: Sim-to-Sim Validation}
...
\begin{itemize}
    \item \textbf{Transfer to a second simulator} (e.g., MuJoCo standalone or Gazebo with a terramechanics plugin) to confirm that escape behaviours are not engine-specific artefacts.
    \item \textbf{Multi-terrain generalization}: sloped sand beds ($\pm$15$^\circ$), variable soil density ($\rho \in [1200, 1600]$\,kg/m$^3$), and lunar gravity ($1.62$\,m/s$^2$) to stress-test policy robustness.
    \item \textbf{Domain randomization expansion}: soil friction coefficient, particle cohesion, motor gain, and observation noise following~\cite{tobin2017domainrand,andrychowicz2020learning}.
    \item \textbf{Multi-seed statistical evaluation}: five random seeds for mean $\pm$ std escape rate and velocity metrics to ensure reproducibility.
\end{itemize}

% ── Priority 3 ──────────────────────────────────────────────────
\subsubsection{Priority 3: Sim-to-Real Transfer \textit{(if possible)}}

Physical deployment is contingent on successful completion of Priority 1--2 and availability of the AAU rover hardware.

\begin{itemize}
    \item \textbf{ONNX export and Raspberry Pi 5 deployment}: export the trained GRU policy to ONNX, validate $<$5\,ms inference at 10\,Hz on RPi5 with ONNX Runtime.
    \item \textbf{Sand tray test}: controlled indoor test in a 1\,m $\times$ 1\,m sand tray with measured sinkage depths matching the training curriculum range (6--12\,cm).
    \item \textbf{Outdoor gravel validation}: unstructured outdoor terrain to evaluate generalization beyond the simulation sand bed geometry.
    \item \textbf{Dual-brain integration}: connect the escape policy with the RLRoverLab navigation policy~\cite{rlroverlab} via the entrapment detector for a complete autonomous traversal and recovery system.
\end{itemize}
