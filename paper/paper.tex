...
training, the policy attains an \textbf{86.9\,\% episode success rate}
(single seed; 3 additional seeds in progress) on a $3.0$\,m projected-escape
threshold, of which $2.75$\,m is in-bed granular traversal and $0.25$\,m is
post-bed slack, with
...
\begin{figure}[!htbp]
  ...
  \caption{Zero-shot cross-engine transfer of the ONNX-exported PPO-GRU actor
    from the Newton MPM\,+\,MuJoCo training domain to three independent Project
    Chrono terrain models, evaluated without any fine-tuning, parameter
    adaptation, or replay buffer. \textbf{CRM} (SPH continuum granular,
    Drucker-Prager plasticity, $n{=}1$): escape in 204 steps at final
    displacement $3.00$\,m. \textbf{SCM} (Bekker-Wong classical terramechanics,
    $n{=}1$): stall at $0.28$\,m after 184 steps - the policy's torque-saturation
    heuristic does not generalise to the fundamentally different
    force-displacement model of classical terramechanics. \textbf{NSC} (rigid
    low-$\mu$ surface, complementarity contact, $n{=}150$ trials over 3 seeds):
    $47.3\%$ recovery rate, 95\% CI $[39.3\%, 55.3\%]$. The pattern - success
    on the SPH-class continuum solver matching the training class, populated
    success statistics on a morphology- and solver-shifted rigid baseline, and
    failure on the model-mismatched Bekker-Wong target - isolates physics
    class as the dominant transfer axis and motivates future adaptation for
    Bekker-Wong terrains.}
...
Four limitations bound the current results.

\textbf{(0)~Single-seed training reported.}
The 86.9\,\% episode success rate and all training-curve figures are computed
from a single seed (\texttt{seed\_1}). Seeds 3, 4, and 5 are currently
training on the same workstation and will be reported in the camera-ready
revision with mean$\pm$std bands across all four seeds. We therefore present
the present numbers as a \emph{single-run baseline} rather than a
