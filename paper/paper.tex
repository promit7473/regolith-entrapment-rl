  ...
  \hspace*{1.2cm}\resizebox{0.92\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[inner, minimum width=3.6cm, minimum height=1cm] (loss) at (4, 0.3)
            {PPO clip loss + value loss\\+ entropy ($\alpha{=}0.08$)};
...
\end{equation}
with $d_{\min}{=}15$\,cm, $d_{\max}{=}28$\,cm. Relative to the $r{=}10$\,cm
wheels this spans rim-deep burial ($1.5r$) to full entombment with $8$\,cm of
overburden and chassis--soil contact ($2.8r$); the upper levels exceed
documented flight-rover embedding (Opportunity at Purgatory Dune reached
approximately full-diameter wheel burial) and constitute a deliberate
worst case. Curriculum
...
% TODO(v12 rewrite): regenerate ALL numbers in this section on the fixed
% physics (substep-dilution fix, measured wheel geometry, honest spawn). The
% scripted suite is now SIX controllers in scripts/escape_eval.py
% (constant_drive / rocking / inching / steer_paddle / rock_paddle / spiral)
% organised in two tiers: Tier 1 = compatible-actuation heuristics, with the
% best TUNED member (scripts/tune_scripted_baseline.sh sweeps period x
% throttle) as the fairness bar; Tier 2 = literature gaits as transferred
% approximations whose failure characterises actuation dependence. The
% spiral egress heuristic is OURS (no published spiral extraction method
% exists; searched 2026-06-13). Rewrite this paragraph to the tier framing
% once the tuned champion is known.
We benchmark the trained policy \emph{in the training engine} (Newton MPM, AAU
rover, Mars gravity) against four non-learned controllers under matched
conditions: \emph{constant drive} (full forward throttle, zero steer); a
scripted \emph{rocking} primitive (alternating full forward/reverse drive,
$2$\,s half-period); a scripted \emph{push--pull inching} gait after
Creager et al.~\cite{creager2015pushpull}, who demonstrated vehicle
extrication from entrapped granular states by thrusting wheel groups against
anchored ones (our rover's rocker is passive, so we approximate the gait by
alternating locked/driving wheel groups); and a scripted \emph{cyclic
steering-sweep} gait after Shrivastava et al.~\cite{shrivastava2020material},
whose rotate-and-sweep ``paddling'' enabled NASA's RP15 robophysical rover to
swim out of entrapping granular media (we use its wheeled reduction:
full-lock sinusoidal steering sweeps superposed on forward drive). Each controller is run for 120 trials (30 per initial
