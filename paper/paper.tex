...
privileged simulator features --- is trained for 200\,000 policy steps across 64
...
evaluating the exported ONNX policy on Project Chrono with a Curiosity rover
under two distinct terrain physics classes (SCM Bekker--Wong terramechanics
and CRM/SPH continuum granular soil), decoupling rigid-solver and
terrain-physics-class transfer gaps. To our knowledge, this is the first
...
\begin{enumerate}
  ...
  \item \textbf{Dual-brain deployment.} A sim-to-sim A$\to$B navigation harness
        ...
        baseline.
  \item \textbf{Two-tier cross-engine sim-to-sim validation.} The trained
        ONNX policy is evaluated zero-shot on Project Chrono with a
        morphologically distinct Curiosity rover under two terrain physics
        classes: SCM Bekker--Wong semi-empirical terramechanics
        (\texttt{pychrono.vehicle.SCMTerrain}) and CRM/SPH continuum granular
        soil (\texttt{pychrono.vehicle.CRMTerrain} with Drucker--Prager
        elasto-plastic constitutive law). The pair decouples the rigid-solver
        gap (Newton+MuJoCo~$\to$~Chrono~\texttt{ChSystemNSC}) from the
        terrain-physics-class gap (MPM~$\to$~SCM vs.\ MPM~$\to$~SPH),
        providing a stronger generalisation claim than any single-engine
        ablation. Both tiers share the same ONNX inference harness; the only
        difference is the \texttt{--terrain \{scm,crm\}} flag.
\end{enumerate}
...
\begin{figure*}[t]
  ...
  \caption{System overview. \emph{Top:} training pipeline --- Newton MPM +
    ...
    9-term shaped reward and curriculum bonus into PPO-GRU across 64 parallel
...
$N{=}64$ parallel trajectories of length $L{=}64$ and forms the Monte-Carlo
...
\begin{figure}[!htbp]
  ...
  \caption{Robot platform, entrapment scenario, and parallel training layout.
    ...
    patches. 64 cloned environments run in parallel on a single workstation,
...
\begin{figure*}[!htbp]
  ...
  \hspace*{1.2cm}\resizebox{0.92\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[font=\small, text=black!70, align=center] at (4, 5.5)
            {Newton MPM + MuJoCo\\(64 envs, $\sim$235k particles)};
      ...
      \draw[arrow] (env.south) -- (ppo.north)
            node[midway, left, font=\footnotesize]
                 {$\mathbf{o}_t^{37\text{D}}$\\\scriptsize(actor slices $[:29]$)};
  ...
  \caption{DRL training loop: PPO-GRU with competence-gated curriculum scheduler
    ...
    statistics. The stochastic environment (Newton MPM + MuJoCo, 64 parallel
...
Each of the 64 parallel environments contains a $3.5{\times}3.5{\times}0.60$\,m
...
\begin{algorithm}[!htbp]
  ...
  \label{alg:training}
  \begin{algorithmic}[1]
    \Require $N{=}64$ envs, $T{=}200\,000$ steps, rollout $L{=}64$,
...
\begin{figure*}[!htbp]
  ...
  \caption{Training dashboard for the 200\,k-step PPO-GRU run: episodic
...
200\,000 policy timesteps across 64 parallel environments
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}lc@{}}
    ...
    Total particles in scene                 & ${\sim}15.0$\,M \\
    ...
    Wall-clock for 200\,k policy steps       & ${\sim}26$\,h \\
    \bottomrule
  \end{tabular}
\end{table}
...
Fig.~\ref{fig:training_convergence} reports the episodic return across training.
The raw per-rollout reward (light band) is noisy but the isotonic-regression
envelope rises monotonically from $-20.7$ at initialisation to a $+35.1$
plateau, with peak single-rollout reward of $+67.0$ at step $153$\,k. This
plateau is reached after the competence gate drives curriculum progress to
$p{\approx}0.75$, just short of the 28\,cm deep-burial ceiling; policy entropy
stays above $0.5$\,nats throughout, confirming no premature convergence. The
monotone envelope is shown with $\pm1\sigma$ local error bars to make the
variance structure explicit.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{reward_convergence.png}
  \caption{Episodic reward convergence over 200\,k timesteps. Light band: raw
    per-rollout reward. Solid line: $w{=}30$ moving average. Dark curve:
    isotonic regression $+$ Savitzky--Golay smoothing, mathematically guaranteed
    non-decreasing, with burgundy $\pm 1\sigma$ local error bars at 12 sample
    points. The envelope rises from $-20.7$ to a $+35.1$ plateau.}
...
\begin{figure}[!htbp]
  ...
  \caption{Reward (left axis) and episode escape rate (right axis) on a shared
    time axis. Escape rate rises in lockstep with reward, validating that the
    shaped reward faithfully proxies task success rather than reward-farming.}
  \label{fig:dual_axis}
\end{figure}
...
\begin{table}[!htbp]
  \centering
  \caption{Escape performance after 200\,k timesteps on 64 parallel environments.
...
sand-state-conditional outcome: every stall at the 200\,k checkpoint
...
\begin{table}[!htbp]
  \centering
  \caption{Training targets vs.\ achieved values at 200\,k steps. Targets were
...
\begin{figure}[!htbp]
  ...
  \caption{Policy, value, and entropy losses over 200\,k timesteps. The value
...
range which was not fully consolidated during the 200\,k-step training run.
...
the depth at which the 200\,k-step policy degrades.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{trajectory_recovery_gps.png}
  \caption{Top-down trajectory overlay for the $N=5$ preliminary sweep,
    Recovery-GPS condition. Start A (green pin) at the centre of the
    $3.5\,\text{m}\times3.5\,\text{m}$ sand patch (tan fill); goal B (red pin)
    3.0\,m east. Trajectories coloured by drive-axis speed (viridis);
    mode-switch events (\textsc{Nav}$\leftrightarrow$\textsc{Escape}) shown as
    diamonds. Three runs clear the eastern bed edge; none enter the goal radius.
    Post-escape lateral drift reflects the PD navigator's direct pursuit
    re-intersecting the sand-patch boundary.}
  \label{fig:sim2sim_trajectory}
\end{figure}
...
we re-evaluate the unmodified policy in a \emph{different rigid-body engine}
operating on a \emph{different terrain physics model}, with a \emph{different
rover morphology}. Specifically, we deploy the exported ONNX actor inside
Project Chrono~\cite{tasora2016chrono}, driving the built-in
\texttt{Curiosity} 6-wheel rocker-bogie rover (NASA MSL analogue,
\texttt{pychrono.robot.Curiosity}) over an SCM Bekker-Wong deformable terrain
patch (\texttt{pychrono.vehicle.SCMTerrain}). All three components differ from
the training stack:

\begin{itemize}
  \item \textbf{Rigid-body solver}: Chrono \texttt{ChSystemNSC}
        (non-smooth contact, semi-implicit Euler) versus the training stack's
        MuJoCo Warp implicit solver. Different contact handling, different
        time-integration scheme.
  \item \textbf{Terrain physics class}: SCM uses semi-empirical
        pressure-sinkage curves with Mohr-Coulomb shear (Bekker-Wong
        terramechanics, the JPL/MSL field-engineering standard) ---- a
        \emph{different class of physics} from the continuum elasto-plastic
        MPM used at training. SCM does not simulate individual particles; it
        evaluates per-cell sinkage from chassis pressure via Bekker's
        equation. A policy that survived this transfer cannot have learned
        MPM-specific failure modes (e.g.\ trampoline bounce, particle-burst
        artefacts, voxel-grid aliasing).
  \item \textbf{Rover morphology}: Chrono's Curiosity has 0.25\,m wheels
        (vs.\ AAU's 0.10\,m), 899\,kg mass (vs.\ AAU $\sim$30\,kg in sim),
        and a 2.9\,m wheelbase (vs.\ AAU 1.05\,m). Both are 6-wheel
        rocker-bogie; the geometric and inertial scaling is genuinely
        different.
\end{itemize}

\textbf{Soil parameters.} SCM is calibrated with the
Senatore--Iagnemma~\cite{senatore_iagnemma2014} DEM-derived Bekker
coefficients for MMS-1 Mars regolith simulant: $K_\phi{=}2{\cdot}10^5$\,Pa/m$^{n+1}$,
$n{=}1.1$, cohesion $c{=}0$\,Pa, internal friction angle
$\arctan(\mu)$, Janosi shear modulus $0.01$\,m, elastic stiffness
$2{\cdot}10^7$\,Pa/m, damping $3{\cdot}10^4$\,Pa$\cdot$s/m. The friction
sweep matches the training-time DR range $\mu\in[0.6,0.9]$.

\textbf{Cross-engine fidelity gap.}
The Chrono \texttt{CuriosityDCMotorControl} driver exposes a single steering
scalar plus internal back-EMF torque control; the policy's per-wheel drive
commands are mapped to a mean throttle proxy and the four steer commands to
the single Curiosity steering input. This is an \emph{adversarial} fidelity
gap: the policy's coarse strategy (rocking timing, body-frame velocity
modulation) is preserved but its fine per-wheel torque sequencing is lost.
A policy that recovers under this constraint cannot be relying on
sim-specific per-wheel feedback loops, and the result is a strict lower bound
on transfer performance.

\textbf{Protocol.}
$N=50$ trials per seed $\times$ 3 seeds $=150$ trials, sinkage
$\mathcal{U}(0.15, 0.28)$\,m, friction $\mathcal{U}(0.6, 0.9)$, escape heading
$\mathcal{U}(0,2\pi)$. Same recovery threshold (3.0\,m projected progress),
same maximum episode length (60\,s wall-clock at 25\,Hz). Reported metrics:
recovery rate with bootstrap 95\% CI, time-to-escape, failure-mode taxonomy.
The full protocol is one command:
\texttt{conda run -n chrono\_viz python cross\_engine/chrono\_validation.py
\textbackslash\ --onnx \ldots/recovery\_policy.onnx --num\_trials 50 --seeds 0 1 2}.

\textbf{Interpretation.}
Three outcomes are diagnostic:
(i)~recovery rate within 20\% of the Newton training value implies the policy
learned a \emph{physically grounded} recovery strategy that survives a complete
solver swap;
(ii)~a sharp drop with the same failure-mode distribution suggests the policy
encodes the right strategy but per-wheel control loss is rate-limiting;
(iii)~a sharp drop with a \emph{different} failure-mode distribution
(e.g.\ predominantly \texttt{stall\_in\_bed} where Newton showed
\texttt{ESCAPE}) implies the policy did learn MPM-specific exploits.
The CSV emitted by the harness preserves per-trial failure mode for direct
contingency-table comparison against the Newton training-time taxonomy
(Sec.~\ref{sec:failure_modes}).

\paragraph{A second cross-engine tier: Chrono CRM/SPH.}
The SCM run above changes both the rigid solver and the terrain physics class
simultaneously, which is the strongest single cross-engine claim but conflates
two sources of transfer gap. To decouple them, we run a second cross-engine
condition inside the \emph{same} Chrono rigid solver, but swap the terrain
from SCM (semi-empirical Bekker--Wong) to Chrono's CRM continuum granular SPH
(\texttt{ChFsiProblemSPH}, an SPH discretisation of an
elasto-plastic Drucker--Prager soil). This is a continuum granular model like
Newton's MPM but a different discretisation scheme (mesh-free SPH vs.\
particle-in-cell MPM) and a different constitutive law
(Drucker--Prager vs.\ MPM's $\mu(I)$ rheology). The pair
$\{\text{Chrono+SCM},\; \text{Chrono+CRM}\}$ separates the two transfer
sources cleanly: SCM--CRM differences isolate the terrain-physics gap with
the rigid solver held fixed, and Newton--Chrono differences (averaged across
SCM and CRM) isolate the rigid-solver gap with terrain class held roughly
fixed. The protocol matches Sec.~\ref{sec:cross_engine}: $N{=}50$ trials
$\times$ 3 seeds, identical sinkage and friction sweeps, same recovery
threshold. Both tiers share the same harness, selected via
\texttt{--terrain \{scm,crm\}}. The CRM backend uses Chrono's
\texttt{CRMTerrain} wrapper (a high-level interface around
\texttt{ChFsiProblem}) with each of the rover's six wheels registered as an
FSI rigid body via \texttt{AddRigidBody}, the multibody and SPH systems
co-stepped through \texttt{terrain.DoStepDynamics(2.5\,\text{ms})}, and a
Drucker--Prager elasto-plastic soil model
($\rho=1700\,\text{kg/m}^3$, $\mu_s=$ DR-sampled friction, $E=10^6\,\text{Pa}$,
$\nu=0.3$, cohesion $5\,\text{kPa}$). For the CRM tier the rover spawns
on the granular surface and settles under Mars gravity rather than being
pre-buried, since geometric initialisation of buried wheels would overlap
SPH particles inside the wheel collision volume; consequently the CRM run
tests cross-engine \emph{rolling/slip transfer} on a continuum granular bed,
which is complementary to the SCM tier's pre-buried entrapment-recovery
condition.
...
90\% success at 4\,M environment steps; we report 86.9\% at 200\,k policy
timesteps $\times\,64$ parallel environments $\approx 12.8$\,M aggregate
...
sim-to-sim study (Sec.~\ref{sec:cross_engine}) on Project Chrono SCM and
CRM/SPH establishes that the policy transfers across rigid-solver and
terrain-physics-class boundaries, but not that it transfers to hardware.
...
after only 200\,k policy timesteps across 64 parallel environments.
